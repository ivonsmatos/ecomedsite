...
const schema = z
  .object({
    ...
    lgpd: z.boolean().refine((v) => v === true, 'Você precisa aceitar os termos.'),
  })
    ...
    <div className="min-h-screen bg-bg flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex items-center gap-2 text-primary font-bold text-2xl font-display">
            <MapPin className="w-7 h-7" />
            EcoMed
          </Link>
          <h1 className="mt-4 text-2xl font-bold font-display text-text">Criar conta</h1>
          <p className="text-text-muted mt-1">Junte-se ao movimento pelo descarte responsável</p>
        </div>

        <div className="bg-surface rounded-2xl shadow-sm border border-gray-100 p-8">
          <button
            onClick={loginWithGoogle}
            className="w-full flex items-center justify-center gap-3 border border-gray-200 py-3 rounded-xl text-text font-medium hover:bg-gray-50 transition-colors mb-6"
          >
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-xs text-text-muted">
              <span className="bg-surface px-3">ou com e-mail</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-text mb-1.5">Senha</label>
              <input
                {...register('senha')}
                type="password"
                placeholder="••••••••"
            ...
            <div>
              <label className="block text-sm font-medium text-text mb-1.5">Confirmar senha</label>
              <input
            ...
            {errors.lgpd && <p className="text-danger text-xs">{errors.lgpd.message}</p>}
